...
printCirc <- read.csv(file = paste0(outFolder, "allCircDOW_df.csv"), sep = ",", )
...
vaccine$doseOne <- vaccine$Primary.mRNA.dose.1
vaccine$fullDose <- vaccine$Primary.mRNA.dose.2 + vaccine$Primary.J.J.Janssen.dose
vaccine$booster <- vaccine$Additional.booster.dose

vaccine %<>% select(c(date, doseOne, fullDose, booster, Population))
...
colnames(data) %<>% recode("totCirc" = "circ_total_physical",
                           ...
                           "doseOne" = "covid_vax_doseone_cnty",
                           "fullDose" = "covid_vax_fulldose_cnty",
...
data %<>% select(-c(allTestResultsStl, probableCasesStl, allTestResultsCnty, probableCasesCnty, program_monthly_total, programs_monthly_attendance))
```
...
data %<>% select(c(date, circ_total_physical, circ_acbk, circ_jcbk, circ_accd, circ_pkbknh, circ_acdvd, circ_acdvd, circ_jccd,
                   ...
                   pc_total_sessions, pc_avg_sessions_hour, 
                   ...
                   covid_vax_doseone_cnty, covid_vax_fulldose_cnty, covid_vax_booster_cnty, covid_vax_population_cnty, pandemic_yes, pandemic_day_number, 
...
```{r}
write.table(x = dataDaily, file = paste0(outFolder, "SPL_Full_Data_Daily_Updated.csv"), quote = FALSE, sep = ",",
